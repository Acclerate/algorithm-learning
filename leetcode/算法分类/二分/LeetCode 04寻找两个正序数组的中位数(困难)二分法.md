...
44 - 创建新数组，长度为两个输入数组长度之和
45 - 分别作为 nums1 和 nums2 的遍历指针
46 - 新数组的写入位置指针
归并阶段（第47-61行）：

第一个 while 循环（第47-55行）：同时遍历两个数组，比较当前指针位置的元素，将较小的放入新数组，并移动对应指针
第二个 while 循环（第56-58行）：处理 nums1 中剩余的元素
第三个 while 循环（第59-61行）：处理 nums2 中剩余的元素
计算中位数（第62-66行）：

如果合并后数组长度为偶数：返回中间两个元素的平均值
如果合并后数组长度为奇数：返回中间元素
3. 重要模式与技术
归并排序的核心思想：利用两个数组已有序的特性，通过双指针技术高效合并，避免完全排序。

双指针技术：使用 lindex 和 rindex 分别指向两个数组的当前位置，实现 O(m+n) 的时间复杂度。

后置自增运算符：a[index++] = nums1[lindex++] 巧妙地同时完成赋值和指针移动。

中位数计算公式：

偶数长度：(a[length/2-1] + a[length/2]) / 2
奇数长度：a[length/2]
注意：此方法空间复杂度为 O(m+n)，因为需要创建额外的合并数组。题目要求的 O(log(m+n)) 时间复杂度需要使用二分法实现（代码后半部分）。
